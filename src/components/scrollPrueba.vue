<template>
  <main class="home-container">
    <section class="text-section">
      <h1>
        <p>Busca tus próximos destinos</p>
        <p>de la forma más</p>
        <span class="highlight">personalizada con IA</span>
      </h1>
      <button :disabled="!process" class="ai-button" @click="changeInterface">{{ buttonText }}</button>
    </section>
    <section class="video-chat-section">
      <!-- <transition name="zoom" appear enter-active-class="animate__animated animate__zoomIn animate__faster"
        leave-active-class="animate__animated animate__zoomOut animate__faster">
        <video v-if="showVideo" ref="video" autoplay loop muted playsinline class="background-video">
      ...
      <transition
          ...
          leave-active-class="animate__animated animate__zoomOut animate__faster"
        >
      <chatPrueba v-if="showChat"  class="chat"/>
      </transition> 
...
import ChatPrueba from '../components/chatPrueba.vue'
import 'animate.css';
...
.video-chat-section {
  ...
  align-items: center;
  position: relative;
  /* Para posicionar el video y el chat correctamente */
}
